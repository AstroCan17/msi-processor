...
.. note:: Consumer-side copy. This module is the same public CCSDS 122.0-B codec the
   producer (s2-msi-raw-generator) implements; the consumer carries its own copy so the
   ground decode (the operational L1A-side decompression) lives in the processing
   facility. Both copies are kept bit-identical (Apache-2.0, same project family).
