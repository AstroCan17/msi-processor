...
| SRF-RU-HER-ENH | `level_1.py::sharpening` (`deconvolution_kernel`), `Denoiser` (`pca`, `moving_avarage_filter`, `wavelet_denoising_cdk`, `gaussian_filter_ips`, `get_filtered_butterworth`, `butterworth_LP_ips`) | `C-PU-ENH` `enhancement.core` *(opt, default-off)* | DPM-M-ENH / ALG-ENH-DECONV,PCA,MA,WAVE,GAUSS,BWLP,FFTDARK | REQ-F-ENH-01..03 | Reuse: deconvolution + denoise/sharpen kernels (radiometry-preserving). Drop: interactive I/O, `matplotlib`. Default-off pending validation (REQ-F-ENH-03). |
...
| ↳ HER-ENH | Reuse algorithm (opt, default-off) | algo ~85 % / code ~10 % | kernels validated before enabling (REQ-F-ENH-03) |
